...
ConvertD3PDToMTreeMC::ConvertD3PDToMTreeMC(TChain *inputChain) : mSUSYD3PD(0), 
                                                                 mOutputTree(0), 
                                                                 mInputChain(inputChain)
{ ; }

void ConvertD3PDToMTreeMC::initialize()
{
  mDoEMJets = true;
  mAnaType = "mc";
  std::transform(mAnaType.begin(), mAnaType.end(), mAnaType.begin(), ::tolower);
  mSUSYD3PD = new SUSYD3PDClass_MC(mInputChain);
  ...
  mPrimaryVertexCollection     = new TClonesArray("MTree::PrimaryVertex", 5); 
...
void ConvertD3PDToMTreeMC::finalize()
{
  mOutputFile->Write();
  mOutputFile->Close();
}

void ConvertD3PDToMTreeMC::run()
{
  ...
  while (mInputChain->GetEntry(entry++))
  {
    ...
    for (int i = 0; i < mSUSYD3PD->el_n; i++)
    {
      ...
      {
        //
        // Add truth information for electron
        //
        MTree::TruthParticle *truthElectron = (MTree::TruthParticle*) mTruthParticleCollection->New(truthParticleCounter++);
        //Some tricks to get mass
        px = py = pz = 0.;
        px = (double)(*mSUSYD3PD->el_truth_pt)[i]/1000.0*TMath::Cos((*mSUSYD3PD->el_truth_phi)[i]);
        py = (double)(*mSUSYD3PD->el_truth_pt)[i]/1000.0*TMath::Sin((*mSUSYD3PD->el_truth_phi)[i]);
        theta = 2*TMath::ATan(TMath::Exp(-1*(*mSUSYD3PD->el_truth_eta)[i]));
        pz = (double)(*mSUSYD3PD->el_truth_pt)[i]/1000.0/TMath::Tan(theta);
        transfer.SetPxPyPzE(px, py, pz, (*mSUSYD3PD->el_truth_E)[i]/1000.0);
        truthElectron->Initialize((double)(*mSUSYD3PD->el_truth_pt)[i]/1000.0,
                                  (double)(*mSUSYD3PD->el_truth_eta)[i],
                                  (double)(*mSUSYD3PD->el_truth_phi)[i],
                                  (double)transfer.M(),
                                  (int)(*mSUSYD3PD->el_truth_type)[i], 
                                  (int)(*mSUSYD3PD->el_truth_status)[i], 
                                  (int)(*mSUSYD3PD->el_truth_mothertype)[i]);
      }
    }
    //
    // Fill Photons
    ...
    for (int i = 0; i < mSUSYD3PD->ph_n; i++)
    {
     ...
     photon->InitializeExtraClusterVariables((double)(*mSUSYD3PD->ph_Ethad1)[i]/1000.0,
      ...
      {
        //
        // Add truth information for photon
        //
        MTree::TruthParticle *truthPhoton = (MTree::TruthParticle*) mTruthParticleCollection->New(truthParticleCounter++);
        //Some tricks to get mass
        px = py = pz = 0.;
        px = (double)(*mSUSYD3PD->ph_truth_pt)[i]/1000.0*TMath::Cos((*mSUSYD3PD->ph_truth_phi)[i]);
        py = (double)(*mSUSYD3PD->ph_truth_pt)[i]/1000.0*TMath::Sin((*mSUSYD3PD->ph_truth_phi)[i]);
        theta = 2*TMath::ATan(TMath::Exp(-1*(*mSUSYD3PD->ph_truth_eta)[i]));
        pz = (double)(*mSUSYD3PD->ph_truth_pt)[i]/1000.0/TMath::Tan(theta);
        transfer.SetPxPyPzE(px, py, pz, (*mSUSYD3PD->ph_truth_E)[i]/1000.0);
        truthPhoton->Initialize((double)(*mSUSYD3PD->ph_truth_pt)[i]/1000.0,
                                (double)(*mSUSYD3PD->ph_truth_eta)[i],
                                (double)(*mSUSYD3PD->ph_truth_phi)[i],
                                (double)transfer.M(),
                                (int)(*mSUSYD3PD->ph_truth_type)[i], 
                                (int)(*mSUSYD3PD->ph_truth_status)[i], 
                                (int)(*mSUSYD3PD->ph_truth_mothertype)[i]);
      }
    }
    //
    // Fill Muons
    ...
    for (int i = 0; i < mSUSYD3PD->mu_staco_n; i++)
    {
      ...
      {
        //
        // Add truth information for muon
        //
        MTree::TruthParticle *truthMuon = (MTree::TruthParticle*) mTruthParticleCollection->New(truthParticleCounter++);
        //Some tricks to get mass
        double px = 0., py = 0., pz = 0., theta = 0.;
        TLorentzVector transfer;
        px = py = pz = 0.;
        px = (double)(*mSUSYD3PD->mu_staco_truth_pt)[i]/1000.0*TMath::Cos((*mSUSYD3PD->mu_staco_truth_phi)[i]);
        py = (double)(*mSUSYD3PD->mu_staco_truth_pt)[i]/1000.0*TMath::Sin((*mSUSYD3PD->mu_staco_truth_phi)[i]);
        theta = 2*TMath::ATan(TMath::Exp(-1*(*mSUSYD3PD->mu_staco_truth_eta)[i]));
        pz = (double)(*mSUSYD3PD->mu_staco_truth_pt)[i]/1000.0/TMath::Tan(theta);
        transfer.SetPxPyPzE(px, py, pz, (*mSUSYD3PD->mu_staco_truth_E)[i]/1000.0);
        truthMuon->Initialize((double)(*mSUSYD3PD->mu_staco_truth_pt)[i]/1000.0,
                                (double)(*mSUSYD3PD->mu_staco_truth_eta)[i],
                                (double)(*mSUSYD3PD->mu_staco_truth_phi)[i],
                                (double)transfer.M(),
                                (int)(*mSUSYD3PD->mu_staco_truth_type)[i], 
                                (int)(*mSUSYD3PD->mu_staco_truth_status)[i], 
                                (int)(*mSUSYD3PD->mu_staco_truth_mothertype)[i]);
      }
    }
    //MUID loop
    ...
    for (int i = 0; i < mSUSYD3PD->mu_muid_n; i++)
    {
      ...
      if (mAnaType == "mc")
      {
        ...
        TLorentzVector transfer;
        px = py = pz = 0.;
        px = (double)(*mSUSYD3PD->mu_muid_truth_pt)[i]/1000.0*TMath::Cos((*mSUSYD3PD->mu_muid_truth_phi)[i]);
        py = (double)(*mSUSYD3PD->mu_muid_truth_pt)[i]/1000.0*TMath::Sin((*mSUSYD3PD->mu_muid_truth_phi)[i]);
        theta = 2*TMath::ATan(TMath::Exp(-1*(*mSUSYD3PD->mu_muid_truth_eta)[i]));
        pz = (double)(*mSUSYD3PD->mu_muid_truth_pt)[i]/1000.0/TMath::Tan(theta);
        transfer.SetPxPyPzE(px, py, pz, (*mSUSYD3PD->mu_muid_truth_E)[i]/1000.0);
        truthMuon->Initialize((double)(*mSUSYD3PD->mu_muid_truth_pt)[i]/1000.0,
                                (double)(*mSUSYD3PD->mu_muid_truth_eta)[i],
                                (double)(*mSUSYD3PD->mu_muid_truth_phi)[i],
                                (double)transfer.M(),
                                (int)(*mSUSYD3PD->mu_muid_truth_type)[i], 
                                (int)(*mSUSYD3PD->mu_muid_truth_status)[i], 
                                (int)(*mSUSYD3PD->mu_muid_truth_mothertype)[i]);
      }
    }
    ...
    for (int i = 0; i < mSUSYD3PD->mu_calo_n; i++)
    {
      ...
      if (mAnaType == "mc")
      {
        ...
        TLorentzVector transfer;
        px = py = pz = 0.;
        px = (double)(*mSUSYD3PD->mu_calo_truth_pt)[i]/1000.0*TMath::Cos((*mSUSYD3PD->mu_calo_truth_phi)[i]);
        py = (double)(*mSUSYD3PD->mu_calo_truth_pt)[i]/1000.0*TMath::Sin((*mSUSYD3PD->mu_calo_truth_phi)[i]);
        theta = 2*TMath::ATan(TMath::Exp(-1*(*mSUSYD3PD->mu_calo_truth_eta)[i]));
        pz = (double)(*mSUSYD3PD->mu_calo_truth_pt)[i]/1000.0/TMath::Tan(theta);
        transfer.SetPxPyPzE(px, py, pz, (*mSUSYD3PD->mu_calo_truth_E)[i]/1000.0);
        truthMuon->Initialize((double)(*mSUSYD3PD->mu_calo_truth_pt)[i]/1000.0,
                                (double)(*mSUSYD3PD->mu_calo_truth_eta)[i],
                                (double)(*mSUSYD3PD->mu_calo_truth_phi)[i],
                                (double)transfer.M(),
                                (int)(*mSUSYD3PD->mu_calo_truth_type)[i], 
                                (int)(*mSUSYD3PD->mu_calo_truth_status)[i], 
                                (int)(*mSUSYD3PD->mu_calo_truth_mothertype)[i]);
      }
    }
...
bool ConvertD3PDToMTreeMC::IsIdenticalTruthParticles(MTree::TruthParticle* particle1, 
...
/*MTree::TruthParticle* ConvertD3PDToMTreeMC::FindParticle(TClonesArray* particles, int pdg_id, int status, 
...
void ConvertD3PDToMTreeMC::SetDaughterLinks(TClonesArray* particles) 
{
  int NrParticles = particles->GetEntriesFast();
  for (int i = 0; i != NrParticles; ++i) 
  {
...
void ConvertD3PDToMTreeMC::FlagDuplicateTruthParticles(TClonesArray* particles) 
{
  int NrParticles = particles->GetEntriesFast();
  for (int i = 0; i != NrParticles; ++i) 
  {
...
void ConvertD3PDToMTreeMC::TransferTruthParticles(TClonesArray* particles) 
{
  ...
  for (int i = 0; i != NrInitialParticles; ++i) 
  {
    ...
    MTree::TruthParticle* particle = (MTree::TruthParticle*) mTruthParticleCollection->New(particleCounter++);
...
MTree::TruthParticle* ConvertD3PDToMTreeMC::GetMatchingTruthParticle(MTree::TruthParticle* inputParticle) 
{
  int NrParticles = mTruthParticleCollection->GetEntriesFast();
  for (int i = 0; i != NrParticles; ++i) 
  {
    MTree::TruthParticle* particle = (MTree::TruthParticle*) mTruthParticleCollection->At(i);
    if (IsIdenticalTruthParticles(inputParticle, particle)) return particle;
  }
  return 0;
}

bool ConvertD3PDToMTreeMC::DumpD3PDLeavesToFile(const std::string& filename)
{
  ...
  if (outfile.fail())
  {
    std::cout << "| ERROR: Cannot open file " << filename 
              << " for writing, in ConvertD3PDToMTreeMC::DumpD3PDLeavesToFile" << std::endl;
    outfile.clear();
    return false;
  }
