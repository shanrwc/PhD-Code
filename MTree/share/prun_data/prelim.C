...
int main(int argc, char **argv)
{
  ...
  system(command.c_str());
  command = "sed -i \'s/.*trig_RoI_EF_e_Rec::/\\/\\/&/g\' " + className + ".h";
  system(command.c_str());
  command = "sed -i \'s/.*trig_RoI_EF_e_Rec::/\\/\\/&/g\' " + className + ".C";
  system(command.c_str());
  command = "sed -i \'s/.*trig_RoI_EF_e_Rec__/\\/\\/&/g\' " + className + ".h";
  system(command.c_str());
  command = "sed -i \'s/.*trig_RoI_EF_e_Rec__/\\/\\/&/g\' " + className + ".C";
  system(command.c_str());
  /*
  if (chan == "data")
  {
