...
//   TFile *HWW = new TFile("root-files/hww/h130_hwwstudy.root");
  ...
  property.push_back("jpt");
  property.push_back("jeta");
  property.push_back("jphi");
  ...
  xtitles.push_back("p_{T}^{jet} [GeV]");
  xtitles.push_back("#eta^{jet}");
  xtitles.push_back("#phi^{jet}");
  ...
  emtitles.push_back("p_{T}^{jet} [GeV]");
  emtitles.push_back("#eta^{jet}");
  emtitles.push_back("#phi^{jet}");
  ...
  for (int nProp = 0; nProp != (int)property.size(); ++nProp)
  {
    for (int nChan = 0; nChan != (int)channels.size(); ++nChan)
    {
      ...
      {
	std::string label = property[nProp] + "_" + channels[nChan] + "1_" + makeString(nPh);
